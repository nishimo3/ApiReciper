...
public class SequenceInfo {
	...
	public String getUniqApiCallLineNuberList(){
		String ret = "";
		List<Integer> posLists = new ArrayList<Integer>();
		for(int i = 0; i < seq.size(); i++){
			ApiInfo apiInfo = seq.get(i);
			if(apiInfo.kind == ApiInfo.KIND_METHOD) {
				posLists.add(apiInfo.lineSPos);
			}
		}

		if(posLists.size() > 0){
			List<Integer> uniqPosLists = new ArrayList<Integer>(new HashSet<>(posLists));
			for(Integer pos : uniqPosLists){
				ret = ret + String.valueOf(pos) + ",";
			}
		}
		return ret;
	}
